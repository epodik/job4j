...
public class Shop {
    ...
    public static void main(String[] args) {
        ...
        System.out.println();
        System.out.println("Заполняем массив и удаляем индекс 2 методом delete");
        products[0] = new Product("Milk", 10);
        products[1] = new Product("Bread", 4);
        products[2] = new Product("Egg", 19);
        products[3] = new Product("Apple", 3);
        products[4] = new Product("Banana", 5);
        Product[] productsDel;
        productsDel = delete(products, 2);
        for (int i = 0; i < productsDel.length; i++) {
            Product product = productsDel[i];
            //проверяем, что объект не равен null. так как  у нас массив не заполнен целиком.
            if (product != null) {
                System.out.println(product.getName());
            } else {
